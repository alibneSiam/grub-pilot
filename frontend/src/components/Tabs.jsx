...
const Tabs = ({ tabs, defaultTab }) => {
  ...
  const updateHeight = () => {
    ...
    if (currentEl) {
      setHeight(currentEl.scrollHeight);
    }
  };
  ...
  useEffect(() => {
    ...
    resizeObserver.current.observe(currentEl);

    return () => {
      if (resizeObserver.current && currentEl) {
        resizeObserver.current.unobserve(currentEl);
      }
    };
  }, [activeTab]);
  ...
  return (
    <div className="flex flex-col w-full">
      <div className="flex my-8 justify-evenly w-full">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`
              flex-1 p-2 font-semibold transition-all duration-500
              ${activeTab === tab.id
